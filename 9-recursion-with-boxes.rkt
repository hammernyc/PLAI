...
(define (num> [l : Value] [r : Value]) : boolean
    (cond
      [(and (numV? l) (numV? r))
       (> (numV-n l) (numV-n r))]
      [else
...
; Interpreter (ExprC -> number) ;
 (define (interp [expr : ExprC] [env : Env] [sto : Store]) : Result
    (type-case ExprC expr
      ...
      [multC (l r) (type-case Result (interp l env sto)
               [v*s (v-l s-l)
                    (type-case Result (interp r env s-l)
                      [v*s (v-r s-r)
                           (v*s (num* v-l v-r) s-r)])])]
      [eqC (l r) (type-case Result (interp l env sto)
               [v*s (v-l s-l)
                    (type-case Result (interp r env s-l)
                      [v*s (v-r s-r)
                           (v*s (num= v-l v-r) s-r)])])]
      ...
      [ifC (c t f) (type-case Result (interp c env sto)
                [v*s (v-c s-c)
                     (if (num> v-c (numV 0))
                         (interp t env s-c)
                         (interp f env s-c))])]
...
(test (eval-exprs (plusS (numS 4) (ifS (numS 1) (numS 2) (numS 4)))) (numV 6))

(test (eval-exprs (plusS (numS 4) (ifS (bminusS (numS 1) (numS 1)) (numS 2) (numS 4)))) (numV 8))

; factorial

(test (eval-exprs (letS 'fact (boxS (numS -1)) (seqS (setboxS (idS 'fact) (lamS 'n (ifS (eqS (idS 'n) (numS 0))
                                                                         (numS 1)
                                                                         (multS (idS 'n) (appS (unboxS (idS 'fact)) (bminusS (idS 'n) (numS 1)))))))
                                      (appS (unboxS (idS 'fact)) (numS 10)))))
      (numV 3628800))